...
export default class CreatorProfile extends Component {
    ...
    getCreatorProfile = async () => {
        ...
        const res = await axios.get(`/api/v1/creator/${creatorId}`)
    ...
    onDeleteCreator = async (creatorId) => {
        await axios.delete(`/api/v1/creator/${creatorId}`)
    }
    ...
    onSubmit = async (evt) => {
        ...
        try {
            const creatorId = this.props.match.params.creatorId
            await axios.put(`/api/v1/creator/${creatorId}`, this.state.creator)
            this.getCreatorProfile()
        } catch (error) {
