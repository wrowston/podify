...
export default class SingleEpisode extends Component {
    ...
    getEpisodeById = async () => {
        ...
        const res = await axios.get(`/api/v1/episode/${episodeId}`)
    ...
    onDeleteEpisode = async (episodeId) => {
        await axios.delete(`/api/v1/episode/${episodeId}`)
    }
    ...
    onSubmit = async (evt) => {
        ...
        try {
            const episodeId = this.props.match.params.episodeId
            await axios.put(`/api/v1/episode/${episodeId}`, this.state.episode)
            this.getEpisodeById()
        } catch (error) {
