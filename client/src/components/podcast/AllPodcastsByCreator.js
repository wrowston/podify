...
export default class AllPodcast extends Component {
    ...
    getAllPodcastsByCreatorId = async () => {
        try {
            ...
            const res = await axios.get(`/api/v1/podcast/creator/${creatorId}`)
    ...
    getCreatorById = async () => {
        const res = await axios.get(`/api/v1/creator/${this.props.creatorId}`)
        const newState = { ...this.state }
    ...
    onSubmit = async (evt) => {
        evt.preventDefault()
        try {
            await axios.post('/api/v1/podcast', this.state.newPodcast)
            this.getAllPodcastsByCreatorId()
        } catch (error) {
                                ...
                                return (
                                    <div key={`t4n328on - ${index}`} className='podcast-wrapper'>
                                        ...
                                        <div className='podcast-info-wrapper'>
                                            <Link to={`/podcast/${podcast.id}`} className='podcast-name'><div>{podcast.name}</div></Link>
